...
# US Cases Model: https://api-app-pjblaypjta-uc.a.run.app/cases/#/
# US Deaths Model: https://api-app-pjblaypjta-uc.a.run.app/deaths/#/
# Model for Cases for a State: https://api-app-pjblaypjta-uc.a.run.app/states/cases/STATE/#/
# Model for Deaths for a State: https://api-app-pjblaypjta-uc.a.run.app/states/deaths/STATE/#
...
def return_raw_data(region):
    ...
    if region_selector == 'USA':
        df = pd.read_json(f"https://api-app-pjblaypjta-uc.a.run.app/API/timeseries/{region}")
    else:
...
def return_model_data(region, days, cases_or_deaths_str):
    df = pd.DataFrame()
    if region_selector == 'USA':
        df = pd.read_json(f"https://modeler-pjblaypjta-uc.a.run.app/{cases_or_deaths_str}/{days}/") #need number of days
    else:
        state_str = region.replace(' ', "%20")
        df = pd.read_json(f"https://modeler-pjblaypjta-uc.a.run.app/states/{cases_or_deaths_str}/{state_str}/{days}/") #need number of days
    return df
